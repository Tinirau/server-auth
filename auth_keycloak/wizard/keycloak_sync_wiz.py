...
class KeycloakSyncMixin(models.AbstractModel):
    ...
    provider_id = fields.Many2one(
        string="Provider", comodel_name="auth.oauth.provider", required=True,
    )
    management_enabled = fields.Boolean(
        related="provider_id.users_management_enabled", readonly=True,
    )
    endpoint = fields.Char(
        related="provider_id.users_endpoint", readonly=True,
    )
    user = fields.Char(related="provider_id.superuser", readonly=True,)
    pwd = fields.Char(related="provider_id.superuser_pwd", readonly=True,)
